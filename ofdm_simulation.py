...
Modulation_type = 'BPSK'  # 调制方式，可选BPSK、QPSK、8PSK、QAM16、QAM64
...
def DeModulation(symbol):
    # 假设接收端已知混沌序列，这里简化处理直接使用相同的混沌序列进行DCSK解调
    chaotic_sequence = generate_chaotic_sequence(len(dataCarriers))
    # DCSK解调
    dcsk_demodulated = symbol * np.exp(-1j * 2 * np.pi * chaotic_sequence)
    if Modulation_type == "QPSK":
        PSK4 = cpy.PSKModem(4)
        bits = PSK4.demodulate(dcsk_demodulated, demod_type='hard')
        return bits
    elif Modulation_type == "QAM64":
        QAM64 = cpy.QAMModem(64)
        bits = QAM64.demodulate(dcsk_demodulated, demod_type='hard')
        return bits
    elif Modulation_type == "QAM16":
        QAM16 = cpy.QAMModem(16)
        bits = QAM16.demodulate(dcsk_demodulated, demod_type='hard')
        return bits
    elif Modulation_type == "8PSK":
        PSK8 = cpy.PSKModem(8)
        bits = PSK8.demodulate(dcsk_demodulated, demod_type='hard')
        return bits
    elif Modulation_type == "BPSK":
        BPSK = cpy.PSKModem(2)
        bits = BPSK.demodulate(dcsk_demodulated, demod_type='hard')
        return bits
...
def OFDM_symbol(QAM_payload):
    ...
    # DCSK调制
    dcsk_payload = QAM_payload * np.exp(1j * 2 * np.pi * chaotic_sequence)

    symbol[dataCarriers] = dcsk_payload  # 在数据位置插入数据
    return symbol
