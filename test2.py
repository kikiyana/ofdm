...
class DRQN(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, attention_dim):
        ...
        self.lstm = nn.LSTM(input_size=state_size, hidden_size=hidden_dim, batch_first=True, bidirectional=True)
        self.attention = SelfAttention(hidden_dim, attention_dim, hidden_dim)
...
# 强化学习环境
class RLNetworkEnv:
    def __init__(self, N, K, num_symbols, snr_db, channel_model='awgn', sequence_length=10):
        self.N = N
        self.K = K
    ...
    def get_state(self):
        ...
        for _ in range(self.sequence_length):
            ...
            _, _, epoch_accuracies = train_model(model, train_loader, 1, 0.001, device)
    ...
    def step(self, action):
        ...
        next_state = np.tile(next_state, (self.sequence_length, 1))
        # 计算奖励
...
class DQNAgent:
    def __init__(self, state_size, action_size, sequence_length=10):
        ...
        self.model = DRQN(state_size, 128, action_size, 64)
        self.target_model = DRQN(state_size, 128, action_size, 64)
...
if __name__ == "__main__":
    ...
    env = RLNetworkEnv(N, K, num_symbols, SNR_db, channel_model, sequence_length)
    state_size = env.current_state.shape[1]
