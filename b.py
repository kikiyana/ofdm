...
# LSTMDemodulator 解调，融入 TCN、注意力机制和 Transformer
class LSTMDemodulator(nn.Module):
    def __init__(self):
        ...
        self.attention = TCNAttention(feature_dim=8)
        self.transformer = TransformerLayer(d_model=16, nhead=2)
    ...
    def forward(self, x):
        feature = self.tcn(x)
        attended_feature = self.attention(feature)
