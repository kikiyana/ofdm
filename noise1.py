...
def generate_dataset(N, K, num_symbols, snr_db, channel_model='awgn', num_pilots=16):
    ...
    if channel_model == 'awgn':
        print('a')
        # received_signal = add_awgn_noise(transmitted_signal, snr_db)
    elif channel_model == 'rayleigh':
        print('a')
        # received_signal = add_rayleigh_noise(transmitted_signal, snr_db)
    elif channel_model == 'multi_path_fast_rayleigh':
        received_signal = add_multi_path_fast_rayleigh_fading(transmitted_signal, snr_db)
    elif channel_model == 'rician':
        received_signal = add_high_speed_railway_fading(transmitted_signal, snr_db)
    else:
...
def add_multi_path_fast_rayleigh_fading(signal, snr_db, num_paths=3, max_delay=5):
    ...
    # 生成符合指数衰减的多径增益
    path_gains = torch.exp(-0.5 * torch.arange(num_paths)).unsqueeze(1).unsqueeze(2)
    path_gains /= torch.sum(path_gains)  # 归一化总功率

    # 生成时延（服从均匀分布）
    delays = torch.randint(0, max_delay + 1, (num_paths,))

    # 生成瑞利衰落系数
    h_real = torch.randn(num_paths, *signal.shape)
    h_imag = torch.randn(num_paths, *signal.shape)
    h = (h_real + 1j * h_imag) / np.sqrt(2)

    # 多径信号合成
    multi_path = torch.zeros_like(signal)
    for i in range(num_paths):
        delayed_signal = torch.roll(signal, int(delays[i]), dims=0)
        multi_path += path_gains[i] * h[i] * delayed_signal

    # 添加AWGN噪声
    noise = torch.sqrt(noise_power / 2) * (torch.randn_like(signal) + 1j * torch.randn_like(signal))
    return multi_path + noise
