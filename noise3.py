...
def generate_dataset(N, K, num_symbols, snr_db, channel_model='awgn'):
    ...
    if channel_model == 'awgn':
        ...
        received_ofdm_symbols = add_high_speed_railway_fading(torch.tensor(ofdm_symbols_with_cp), snr_db)
    else:
...
def add_multi_path_fast_rayleigh_fading(signal, snr_db, num_paths=3, max_delay=5):
    ...
    # 生成每个路径的瑞利衰落增益（幅度）
    gains = torch.empty(num_paths).normal_(0, 1)  # 衰落幅度，服从瑞利分布
    # 生成每个路径的相位（均匀分布）
    phases = 2 * np.pi * torch.rand(num_paths)
    ...
    for i in range(num_paths):
        ...
        # 生成该路径的瑞利衰落系数（复数形式）
        h_real = torch.normal(0, 1, signal.size())
        h_imag = torch.normal(0, 1, signal.size())
        h = (h_real + 1j * h_imag) / np.sqrt(2)  # 幅度归一化

        # 应用复增益和相位
        h *= gain * torch.exp(1j * phase)

        # 处理延时（前补零）
        if delay > 0:
            if signal.dim() == 1:  # 一维信号
                delay_zero = torch.zeros(delay, dtype=torch.complex64)
                delayed_signal = signal[:-delay]
                delayed_signal = torch.cat((delay_zero, delayed_signal))
            elif signal.dim() == 2:  # 二维信号（例如 batch × time）
                delay_zero = torch.zeros((signal.shape[0], delay), dtype=torch.complex64)
                delayed_signal = signal[:, :-delay]
                delayed_signal = torch.cat((delay_zero, delayed_signal), dim=1)
        else:
            delayed_signal = signal

        # 应用衰落系数到延时后的信号
        path_signal = h * delayed_signal

        # 叠加到多径信号上
        multi_path_signal += path_signal

    # 添加高斯噪声
    noise_real = torch.normal(0, np.sqrt(noise_power / 2), signal.size())
    noise_imag = torch.normal(0, np.sqrt(noise_power / 2), signal.size())
...
if __name__ == "__main__":
    ...
    N = 64  # OFDM 子载波数量:64
    K = 128  # 混沌序列长度:128
    # num_symbols_list = [1000, 10000, 100000]  # 数据符号数量列表
    num_symbols_list = [10000]  # 数据符号数量列表
    # SNR_dbs = [0, 5, 10, 15, 20]  # 不同信噪比 (dB)
    SNR_dbs = [20]  # 不同信噪比 (dB)
    channel_models = ['rician']  # 信道模型列表
    # channel_models = ['awgn', 'rayleigh', 'multi_path_fast_rayleigh', 'rician']  # 信道模型列表
    ...
    for channel_model in channel_models:
        ...
        for snr_db in SNR_dbs:
            ...
            for num_symbols in num_symbols_list:
                ...
                # 存储结果
                results[channel_model][snr_db][num_symbols] = {
                ...
                end_time = time.time()
                print(f"{channel_model} - SNR: {snr_db}dB - Num Symbols: {num_symbols} - Total Time: {end_time - start_time:.2f}s")

    # # 将结果保存为 JSON 文件
    # with open('results_TDRQN_Dual_TCN.json', 'w') as f:
