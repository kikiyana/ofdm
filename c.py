#去掉TCNAttention,只用TCN和transformer，
...
# 定义注意力机制模块，用于融合 TCN 的特征（这里简化，可根据需求调整）
class TCNAttention(nn.Module):
    def __init__(self, feature_dim):
        ...
        self.value = nn.Linear(feature_dim, feature_dim // 2)
        self.softmax = nn.Softmax(dim=-1)

    def forward(self, feature):
        # 调整维度，将通道维度和序列长度维度合并
        batch_size, channels, seq_len = feature.size()
        feature = feature.transpose(1, 2).contiguous().view(batch_size * seq_len, channels)

        query = self.query(feature).view(batch_size, seq_len, -1).transpose(1, 2).contiguous()
        key = self.key(feature).view(batch_size, seq_len, -1).transpose(1, 2).contiguous()
        value = self.value(feature).view(batch_size, seq_len, -1).transpose(1, 2).contiguous()
...
class LSTMDemodulator(nn.Module):
    def __init__(self):
        ...
        self.tcn = TCN(input_channels=1, num_channels=[4, 8], kernel_size=3, dilation_base=2)
        self.transformer = TransformerLayer(d_model=8, nhead=2)  # 注意这里输入维度修改为 8
        self.bilstm = BiLSTM(8, 128, 2)  # 注意这里输入维度修改为 8
    ...
    def forward(self, x):
        feature = self.tcn(x)
        feature = feature.transpose(1, 2)  # 调整维度以适应 Transformer 输入
        feature = self.transformer(feature)
        x = self.bilstm(feature)
        x = x[:, -1, :]
        x = F.relu(self.fc1(x))
        x = self.bn(x)
        x = self.fc2(x)
