...
def add_nakagami_m_fading(signal, snr_db, m):
    """添加 Nakagami - m 衰落和噪声"""
    snr_linear = 10 ** (snr_db / 10)
    signal_power = torch.mean(torch.abs(signal) ** 2)
    noise_power = signal_power / snr_linear
    ...
    # 生成 Nakagami - m 衰落系数
    h_magnitude = torch.sqrt(torch.distributions.gamma.Gamma(m, m).sample(signal.size()))
    h_phase = 2 * np.pi * torch.rand(signal.size())
    h = h_magnitude * torch.exp(1j * h_phase)
...
if __name__ == "__main__":
    ...
    num_symbols_list = [50000]  # 数据符号数量列表
    # SNR_dbs = [0, 5, 10, 15, 20]  # 不同信噪比 (dB)
    SNR_dbs = [10]  # 不同信噪比 (dB)
    channel_models = ['multi_path_fast_rayleigh']  # 信道模型列表
