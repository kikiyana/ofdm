...
if __name__ == "__main__":
            ...
            for epoch in range(num_epochs):
                for i, (inputs, labels, _) in enumerate(train_loader):
                    ...
                    with torch.no_grad():
                        ...
                        # 这里需要手动模拟获取 BiLSTM 层的输出
                        # 可以参考 LSTMDemodulator 类的 forward 方法
                        local_feature = trained_model.local_tcn(inputs)
                        global_feature = trained_model.global_tcn(inputs)
                        attended_feature = trained_model.attention(local_feature, global_feature)
                        attended_feature = attended_feature.transpose(1, 2)
                        attended_feature = trained_model.transformer(attended_feature)
                        features = trained_model.bilstm(attended_feature)[:, -1, :]  # 取最后一个时间步的输出
    ...
    # 将结果保存为 JSON 文件
    with open('results_rpsr_dqn_Dual_TCN.json', 'w') as f:
