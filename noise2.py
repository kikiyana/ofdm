...
def generate_dataset(N, K, num_symbols, snr_db, channel_model='awgn'):
    ...
    if channel_model == 'awgn':
        ...
        received_ofdm_symbols = add_multi_path_fast_rayleigh_fading(torch.tensor(ofdm_symbols_with_cp), snr_db)
...
def add_multi_path_fast_rayleigh_fading(signal, snr_db, num_paths=3, max_delay=5):
    ...
    # 生成多径参数
    delays = torch.randint(0, max_delay, (num_paths,))
    path_gains = torch.sqrt(torch.tensor(0.5)) * torch.randn(num_paths, *signal.shape)

    # 多径叠加
    multi_path = torch.zeros_like(signal)
    for i in range(num_paths):
        delayed = torch.roll(signal, int(delays[i]), dims=0)
        multi_path += path_gains[i] * delayed

    # 添加噪声
    noise = torch.sqrt(noise_power) * (torch.randn_like(signal) + 1j * torch.randn_like(signal))
